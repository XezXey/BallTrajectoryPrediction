...
def train(output_trajectory_train, output_trajectory_train_mask, output_trajectory_train_lengths, output_trajectory_train_startpos, output_trajectory_train_xyz, input_trajectory_train, input_trajectory_train_mask, input_trajectory_train_lengths, input_trajectory_train_startpos, generator, discriminator, hidden_D, cell_state_D, hidden_G, cell_state_G, output_trajectory_val, output_trajectory_val_mask, output_trajectory_val_lengths, output_trajectory_val_startpos, output_trajectory_val_xyz, input_trajectory_val, input_trajectory_val_mask, input_trajectory_val_lengths, input_trajectory_val_startpos, projection_matrix, camera_to_world_matrix, epoch, n_epochs, vis_signal, optimizer_D, optimizer_G, width, height, train_generator, train_discriminator, visualize_trajectory_flag=True, visualization_path='./visualize_html/'):
  ...
  output_train, (_, _) = generator(input_trajectory_train, hidden_G, cell_state_G, lengths=input_trajectory_train_lengths)
  ...
  gravity_loss = GravityLoss(output=output_train_xyz.clone(), trajectory_gt=output_trajectory_train_xyz[..., :-1], mask=output_trajectory_train_mask[..., :-1], lengths=output_trajectory_train_lengths)
  ...
  output_val, (_, _) = generator(input_trajectory_val, hidden_G, cell_state_G, lengths=input_trajectory_val_lengths)
  # (This step we get the displacement of depth by input the displacement of u and v)
  # Apply cummulative summation to output using cumsum_trajectory function
  ...
  val_loss = TrajectoryLoss(output=output_val_xyz, trajectory_gt=output_trajectory_val_xyz[..., :-1], mask=output_trajectory_val_mask[..., :-1], lengths=output_trajectory_val_lengths) + GravityLoss(output=output_val_xyz.clone(), trajectory_gt=output_trajectory_val_xyz[..., :-1], mask=output_trajectory_val_mask[..., :-1], lengths=output_trajectory_val_lengths)
  ...
  return loss_G.item(), loss_D.item(), val_loss.item(), (hidden_G_eot, hidden_G_depth), (cell_state_G_eot, cell_state_G_depth), hidden_D, cell_state_D, generator_eot, generator_depth, discriminator
...
if __name__ == '__main__':
  ...
  # Init wandb
  wandb.init(project="ball-trajectory-estimation", name=args.wandb_name, tags=args.wandb_tags, notes=args.wandb_notes, dir='./')
  ...
  for epoch in range(start_epoch, n_epochs+1):
    ...
    for batch_idx, batch_train in enumerate(trajectory_train_dataloader):
      ...
      loss_G, loss_D, val_loss, hidden_G, cell_state_G, hidden_D, cell_state_D, generator_eot, generator_depth, discriminator = train(output_trajectory_train=output_trajectory_train, output_trajectory_train_mask=output_trajectory_train_mask,
                                                                 ...
                                                                 generator_eot=generator_eot, generator_depth=generator_depth, hidden_G=hidden_G, cell_state_G=cell_state_G, discriminator=discriminator, hidden_D=hidden_D, cell_state_D=cell_state_D,
