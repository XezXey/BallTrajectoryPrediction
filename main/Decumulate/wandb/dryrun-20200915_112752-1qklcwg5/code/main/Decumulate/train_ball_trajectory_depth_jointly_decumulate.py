...
def GravityLoss(output, trajectory_gt, mask, lengths):
  ...
  for i in range(trajectory_gt.shape[0]):
    ...
    if trajectory_gt[i][:lengths[i], 1].shape[0] < 6:
      ...
      continue
    print(trajectory_gt[i][:lengths[i], 1].shape)
...
def split_cumsum(reset_idx, length, start_pos, reset_depth, depth, eot):
  ...
  if reset_idx[-1] != depth.shape[0] and reset_idx.shape[0] > 1:
    reset_idx[-1] = max_len
  print(reset_idx, eot[reset_idx])
  print(eot.shape, depth.shape)
  print([[start, end] if start==0 else [start+1, end] for start, end in zip(reset_idx, reset_idx[1:])])
  ...
  for each_depth_chunk in depth_chunk:
    print(each_depth_chunk.shape)
