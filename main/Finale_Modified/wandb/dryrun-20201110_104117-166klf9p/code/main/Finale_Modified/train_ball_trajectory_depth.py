...
def train(input_train_dict, gt_train_dict, input_val_dict, gt_val_dict, model_list, epoch, vis_signal, optimizer, cam_params_dict, vis_flag=True, visualization_path='./visualize_html/'):
  ...
  ############# Training #############
  ####################################
  utils_model.train_mode(model_list=model_list)

  pred_list_train = utils_model.fw_pass(model_list, input_dict=input_train_dict)
  ...
  train_loss_dict, train_loss = utils_model.calculate_loss(pred_xyz=pred_xyz_train, gt_dict=gt_train_dict, cam_params_dict=cam_params_dict, pred_list=pred_list_train) # Calculate the loss
  ...
  ####################################
  # Evaluating mode
  utils_model.eval_mode(model_list=model_list)

  pred_list_val = utils_model.fw_pass(model_list, input_dict=input_val_dict)
  if args.pipeline == ['eot', 'depth']:
  ...
  optimizer.zero_grad() # Clear existing gradients from previous epoch
  val_loss_dict, val_loss = utils_model.calculate_loss(pred_xyz=pred_xyz_val, gt_dict=gt_val_dict, cam_params_dict=cam_params_dict, pred_list=pred_list_train) # Calculate the loss
...
def load_checkpoint(model_list, optimizer, lr_scheduler):
  ...
  if os.path.isfile(load_checkpoint):
    ...
    for idx, model in enumerate(checkpoint['model_cfg'].keys()):
      model_list[idx].load_state_dict(checkpoint['model_{}'.format(model)])

    optimizer.load_state_dict(checkpoint['optimizer'])
    lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
    start_epoch = checkpoint['epoch']
    min_val_loss = checkpoint['min_val_loss']
    return model_list, optimizer, start_epoch, lr_scheduler, min_val_loss
...
if __name__ == '__main__':
  ...
  # Model definition
  min_val_loss = 2e10
  model_list, model_cfg = utils_func.get_model_depth(model_arch=args.model_arch, features_cols=features_cols, args=args)
  print(model_list)
  print(model_cfg)
  ...
  for model in model_list:
    params += list(model.parameters())
  params_2 = list(model_list[0].parameters()) + list(model_list[1].parameters())
  ...
  if args.load_checkpoint is None:
    ...
    model_list, optimizer, start_epoch, lr_scheduler, min_val_loss = load_checkpoint(model_list, optimizer, lr_scheduler)
  ...
  for epoch in range(start_epoch, n_epochs+1):
    ...
    for batch_idx, batch_train in enumerate(trajectory_train_dataloader):
      ...
      train_loss, val_loss, model_flag, model_depth = train(input_train_dict=input_train_dict, gt_train_dict=gt_train_dict, input_val_dict=input_val_dict, gt_val_dict=gt_val_dict,
                                                            model_list=model_list, vis_flag=args.vis_flag,
