...
def train(input_train_dict, gt_train_dict, input_val_dict, gt_val_dict, model_dict, epoch, vis_signal, optimizer, cam_params_dict, vis_flag=True, visualization_path='./visualize_html/'):
  ...
  pred_dict_train = utils_model.fw_pass(model_dict, input_dict=input_train_dict)
  if args.pipeline == ['eot', 'depth']:
    pred_flag_train = pred_dict_train['model_flag'][0]
    pred_depth_train = pred_dict_train['model_depth'][0]
  ...
  pred_dict_val = utils_model.fw_pass(model_dict, input_dict=input_val_dict)
  if args.pipeline == ['eot', 'depth']:
    pred_flag_val = pred_dict_val['model_flag'][0]
    pred_depth_val = pred_dict_val['model_depth'][0]
  elif args.pipeline == ['depth']:
    pred_depth_val = pred_dict_val['model_depth'][0]
    pred_eot_val = None
