...
def train(input_train_dict, gt_train_dict, input_val_dict, gt_val_dict, model_depth, epoch, vis_signal, optimizer, cam_params_dict, vis_flag=True, visualization_path='./visualize_html/'):
  ...
  if args.latent:
    ...
    in_train = pt.cat((in_train*0., input_train_dict['input'][:, [0], 2:].repeat(1, input_train_dict['input'].shape[1], 1)), dim=2)  # Concat the (u_noise, v_noise, pred_eot, other_features(col index 3+)
