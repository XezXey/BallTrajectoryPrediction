...
def add_noise(input_trajectory, startpos, lengths):
  ...
  print(startpos[0])
  for i in range(input_trajectory.shape[0]):
    print(input_trajectory[i][0])
  exit()
...
def flag_metrics_log(flag_gt, output_flag, lengths, flag):
  ...
  wandb.log({'{} Precision'.format(flag):precision, '{} Recall'.format(flag):recall, '{} F1-score'.format(flag):f1_score, '{}-#N accepted trajectory(Perfect flag without FN, FP)'.format(flag):n_accepted_trajectory})

def FlagLoss(output_flag, flag_gt, flag_startpos, mask, lengths, flag='Train'):
  # Here we use output mask so we need to append the startpos to the output_flag before multiplied with mask(already included the startpos)
...
def train(output_trajectory_train, output_trajectory_train_mask, output_trajectory_train_lengths, output_trajectory_train_startpos, output_trajectory_train_xyz, input_trajectory_train, input_trajectory_train_mask, input_trajectory_train_lengths, input_trajectory_train_startpos, model_flag, model_depth, output_trajectory_val, output_trajectory_val_mask, output_trajectory_val_lengths, output_trajectory_val_startpos, output_trajectory_val_xyz, input_trajectory_val, input_trajectory_val_mask, input_trajectory_val_lengths, input_trajectory_val_startpos, projection_matrix, camera_to_world_matrix, epoch, n_epochs, vis_signal, optimizer, width, height, visualize_trajectory_flag=True, visualization_path='./visualize_html/'):
  ...
  print(output_train_flag)
  print(output_train_flag.shape)
  exit()
  ...
  if args.decumulate and epoch > args.start_decumulate:
    output_train_depth_cumsum, input_trajectory_train_uv_cumsum = cumsum_decumulate_trajectory(depth=output_train_depth, uv=input_trajectory_train_gt[..., :-1], trajectory_startpos=input_trajectory_train_startpos, lengths=input_trajectory_train_lengths, flag=input_trajectory_train_gt[..., -1].unsqueeze(dim=-1), projection_matrix=projection_matrix, camera_to_world_matrix=camera_to_world_matrix, width=width, height=height)
  ...
  train_eot_loss = FlagLoss(output_flag=output_train_flag, flag_gt=input_trajectory_train_gt[..., -1], mask=input_trajectory_train_mask[..., -1], lengths=input_trajectory_train_lengths, flag_startpos=input_trajectory_train_startpos[..., -1], flag='Train')
  train_onground_loss = FlagLoss(output_flag=output_train_flag, flag_gt=input_trajectory_train_gt[..., -2], mask=input_trajectory_train_mask[..., -2], lengths=input_trajectory_train_lengths, flag_startpos=input_trajectory_train_startpos[..., -2], flag='Train')
  ...
  val_eot_loss = FlagLoss(output_flag=output_val_flag, flag_gt=input_trajectory_val_gt[..., -1], mask=input_trajectory_val_mask[..., -1], lengths=input_trajectory_val_lengths, flag_startpos=input_trajectory_val_startpos[..., -1], flag='Validation')
  val_onground_loss = FlagLoss(output_flag=output_val_flag, flag_gt=input_trajectory_val_gt[..., -2], mask=input_trajectory_val_mask[..., -2], lengths=input_trajectory_val_lengths, flag_startpos=input_trajectory_val_startpos[..., -2], flag='Validation')
