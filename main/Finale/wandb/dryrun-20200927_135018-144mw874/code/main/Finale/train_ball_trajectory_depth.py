...
def train(input_train_dict, gt_train_dict, input_val_dict, gt_val_dict, model_flag, model_depth, epoch, vis_signal, optimizer, cam_params_dict, vis_flag=True, visualization_path='./visualize_html/'):
  ...
  val_eot_loss = loss.EndOfTrajectoryLoss(pred=pred_eot_val, gt=gt_val_dict['d_with_f'][..., [1]], mask=input_val_dict['mask'][..., [2]], lengths=input_val_dict['lengths'], startpos=input_val_dict['startpos'][..., [2]], flag='val')
  val_depth_loss = loss.DepthLoss(pred=pred_depth_val, gt=gt_val_dict['d_with_f'][..., [0]], lengths=input_val_dict['lengths'], mask=input_val_dict['mask'])
  ...
  if vis_flag == True and vis_signal == True:
    ...
    utils_func.make_visualize(input_train_dict=input_train_dict, gt_train_dict=gt_train_dict, input_val_dict=input_val_dict, gt_val_dict=gt_val_dict, pred_train_dict=pred_train_dict, pred_val_dict=pred_val_dict, visualization_path=visualization_path)
...
def collate_fn_padd(batch):
    ...
    print(pt.Tensor(batch[0][..., [x, y, z]]).shape)
    # print(pt.cumsum(pt.Tensor(batch[0][..., [x, y, z]]), dim=1).shape)
    # print("SUM : ", pt.cumsum(pt.Tensor(batch[0][..., [x, y, z]]), dim=1))
    # print("NOT SUM : ", pt.Tensor(batch[0][..., [x, y, z]]))
    exit()
