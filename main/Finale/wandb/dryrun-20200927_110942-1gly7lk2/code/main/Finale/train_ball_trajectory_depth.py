...
def train(input_train_dict, gt_train_dict, input_val_dict, gt_val_dict, model_flag, model_depth, epoch, vis_signal, optimizer, cam_params_dict, vis_flag=True, visualization_path='./visualize_html/'):
  ...
  train_eot_loss = loss.EndOfTrajectoryLoss(pred=pred_eot_train, gt=gt_train_dict['gt'][..., [1]], mask=input_train_dict['mask'][..., [2]], lengths=input_train_dict['lengths'], startpos=input_train_dict['startpos'][..., [2]], flag='Train')
  train_depth_loss = loss.DepthLoss(pred=pred_depth_train, gt=gt_train_dict['gt'][..., [0]], lengths=input_train_dict['lengths'], mask=input_train_dict['mask'])
  ...
  pred_depth_cumsum_val, input_uv_cumsum_val = utils_cummulative.cummulative_fn(depth=pred_depth_val, uv=input_val_dict['input'][..., [0, 1]], depth_teacher=gt_val_dict['gt'][..., [0]], startpos=input_val_dict['startpos'], lengths=input_val_dict['lengths'], eot=input_val_dict['input'][..., [2]], cam_params_dict=cam_params_dict, epoch=epoch, args=args)

  # Project the (u, v, depth) to world space
  pred_xyz_val = pt.stack([utils_transform.projectToWorldSpace(uv=input_uv_cumsum_val[i], depth=pred_depth_val[i], cam_params_dict=cam_params_dict, device=device) for i in range(input_uv_cumsum_val.shape[0])])
  ...
  val_eot_loss = loss.EndOfTrajectoryLoss(pred=pred_eot_val, gt=gt_val_dict['gt'][..., [1]], mask=input_val_dict['mask'][..., [2]], lengths=input_val_dict['lengths'], startpos=input_val_dict['startpos'][..., [2]], flag='val')
  val_depth_loss = loss.DepthLoss(pred=pred_depth_val, gt=gt_val_dict['gt'][..., [0]], lengths=input_val_dict['lengths'], mask=input_val_dict['mask'])
  ...
  # Sum up all val loss 
  val_loss = val_trajectory_loss + val_eot_loss*100 + val_depth_loss*1000 + val_gravity_loss + val_below_ground_loss
...
def collate_fn_padd(batch):
    ...
    gt_xyz = [pt.cat((pt.cumsum(pt.Tensor(trajectory[:, [x, y, z]]), dim=1), pt.Tensor(trajectory[..., features_cols])), dim=-1) for trajectory in batch]
...
if __name__ == '__main__':
  ...
  for epoch in range(start_epoch, n_epochs+1):
    ...
    val_input = {'input':batch_val['input'][0].to(device), 'lengths':batch_val['input'][1].to(device), 'mask':batch_val['input'][2].to(device), 'startpos':batch_val['input'][3].to(device)}
    val_gt = {'d_with_f':batch_val['gt'][0].to(device), 'lengths':batch_val['gt'][1].to(device), 'mask':batch_val['gt'][2].to(device), 'startpos':batch_val['gt'][3].to(device), 'xyz':batch_val['gt'][4].to(device)}
    ...
    for batch_idx, batch_train in enumerate(trajectory_train_dataloader):
      ...
      train_input = {'input':batch_train['input'][0].to(device), 'lengths':batch_train['input'][1].to(device), 'mask':batch_train['input'][2].to(device), 'startpos':batch_train['input'][3].to(device)}
      train_gt = {'d_with_f':batch_train['gt'][0].to(device), 'lengths':batch_train['gt'][1].to(device), 'mask':batch_train['gt'][2].to(device), 'startpos':batch_train['gt'][3].to(device), 'xyz':batch_train['gt'][4].to(device)}

      # Call function to train
      train_loss, val_loss, model_flag, model_depth = train(train_input_dict=train_input, train_gt_dict=train_gt, val_input_dict=val_input, val_gt_dict=val_gt,
