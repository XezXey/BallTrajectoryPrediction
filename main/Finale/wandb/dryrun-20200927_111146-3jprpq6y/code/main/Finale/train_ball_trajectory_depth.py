...
if __name__ == '__main__':
  ...
  for epoch in range(start_epoch, n_epochs+1):
    ...
    gt_val_dict = {'d_with_f':batch_val['gt'][0].to(device), 'lengths':batch_val['gt'][1].to(device), 'mask':batch_val['gt'][2].to(device), 'startpos':batch_val['gt'][3].to(device), 'xyz':batch_val['gt'][4].to(device)}
    ...
    for batch_idx, batch_train in enumerate(trajectory_train_dataloader):
      ...
      gt_train_dict = {'d_with_f':batch_train['gt'][0].to(device), 'lengths':batch_train['gt'][1].to(device), 'mask':batch_train['gt'][2].to(device), 'startpos':batch_train['gt'][3].to(device), 'xyz':batch_train['gt'][4].to(device)}
