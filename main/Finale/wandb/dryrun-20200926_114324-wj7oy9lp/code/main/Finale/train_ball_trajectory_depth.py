...
def make_visualize(input_trajectory_train, output_train_depth, input_trajectory_val, output_val_depth, output_train_xyz, output_trajectory_train_xyz, output_trajectory_train_startpos, input_trajectory_train_lengths, output_trajectory_train_maks, output_val_xyz, output_trajectory_val_xyz, output_trajectory_val_startpos, input_trajectory_val_lengths, output_trajectory_val_mask, visualization_path):
  ...
  visualize_trajectory(output=pt.mul(output_val_xyz, output_trajectory_val_mask[..., [0, 1, 2]]), trajectory_gt=output_trajectory_val_xyz[..., [0, 1, 2]], lengths=input_trajectory_val_lengths, mask=output_trajectory_val_mask[..., [0, 1, 2]], fig=fig_traj, flag='Validation', n_vis=n_vis, vis_idx=val_vis_idx)
  ...
  visualize_eot(output_eot=input_trajectory_train[..., 2].clone(), eot_gt=output_trajectory_train_xyz[..., 3], eot_startpos=output_trajectory_train_startpos[..., 3], lengths=output_trajectory_train_lengths, mask=output_trajectory_train_mask[..., 3], fig=fig_eot, flag='Train', n_vis=n_vis, vis_idx=train_vis_idx)
  visualize_eot(output_eot=input_trajectory_val[..., 2].clone(), eot_gt=output_trajectory_val_xyz[..., 3], eot_startpos=output_trajectory_val_startpos[..., 3], lengths=output_trajectory_val_lengths, mask=output_trajectory_val_mask[..., 3], fig=fig_eot, flag='Validation', n_vis=n_vis, vis_idx=val_vis_idx)
...
def visualize_trajectory(output, trajectory_gt, lengths, mask, vis_idx, fig=None, flag='train', n_vis=5):
...
def train(output_trajectory_train, output_trajectory_train_mask, output_trajectory_train_lengths, output_trajectory_train_startpos, output_trajectory_train_xyz, input_trajectory_train, input_trajectory_train_mask, input_trajectory_train_lengths, input_trajectory_train_startpos, model_flag, model_depth, output_trajectory_val, output_trajectory_val_mask, output_trajectory_val_lengths, output_trajectory_val_startpos, output_trajectory_val_xyz, input_trajectory_val, input_trajectory_val_mask, input_trajectory_val_lengths, input_trajectory_val_startpos, epoch, n_epochs, vis_signal, optimizer, cam_params_dict, visualize_trajectory_flag=True, visualization_path='./visualize_html/'):
  ...
  if args.decumulate and epoch > args.start_decumulate:
    output_train_depth_cumsum, input_trajectory_train_uv_cumsum = cumsum_decumulate_trajectory(depth=output_train_depth, uv=input_trajectory_train_gt[..., [0, 1]], trajectory_startpos=input_trajectory_train_startpos, lengths=input_trajectory_train_lengths, eot=input_trajectory_train_gt[..., [2]], projection_matrix=projection_matrix, camera_to_world_matrix=camera_to_world_matrix, width=width, height=height)
  ...
  output_train_xyz = pt.stack([projectToWorldSpace(uv=input_trajectory_train_uv_cumsum[i], depth=output_train_depth_cumsum[i], projection_matrix=projection_matrix, camera_to_world_matrix=camera_to_world_matrix, width=width, height=height) for i in range(output_train_depth.shape[0])])

  ####################################
  ############# EOT&Depth ############
  ####################################
  optimizer.zero_grad() # Clear existing gradients from previous epoch
  train_trajectory_loss = TrajectoryLoss(output=output_train_xyz, trajectory_gt=output_trajectory_train_xyz[..., [0, 1, 2]], mask=output_trajectory_train_mask[..., [0, 1, 2]], lengths=output_trajectory_train_lengths)
  train_gravity_loss = GravityLoss(output=output_train_xyz, trajectory_gt=output_trajectory_train_xyz[..., [0, 1, 2]], mask=output_trajectory_train_mask[..., [0, 1, 2]], lengths=output_trajectory_train_lengths)
  train_eot_loss = EndOfTrajectoryLoss(output_eot=output_train_eot, eot_gt=input_trajectory_train_gt[..., [2]], mask=input_trajectory_train_mask[..., [2]], lengths=input_trajectory_train_lengths, eot_startpos=input_trajectory_train_startpos[..., [3]], flag='Train')
  train_depth_loss = DepthLoss(output=output_train_depth, depth_gt=output_trajectory_train[..., [0]], lengths=input_trajectory_train_lengths, mask=input_trajectory_train_mask)
  train_below_ground_loss = BelowGroundPenalize(output=output_train_xyz, trajectory_gt=output_trajectory_train_xyz[..., [0, 1, 2]], mask=output_trajectory_train_mask[..., [0, 1, 2]], lengths=output_trajectory_train_lengths)
  ...
  val_trajectory_loss = TrajectoryLoss(output=output_val_xyz, trajectory_gt=output_trajectory_val_xyz[..., [0, 1, 2]], mask=output_trajectory_val_mask[..., [0, 1, 2]], lengths=output_trajectory_val_lengths)
  val_gravity_loss = GravityLoss(output=output_val_xyz, trajectory_gt=output_trajectory_val_xyz[..., [0, 1, 2]], mask=output_trajectory_val_mask[..., [0, 1, 2]], lengths=output_trajectory_val_lengths)
  val_eot_loss = EndOfTrajectoryLoss(output_eot=output_val_eot, eot_gt=input_trajectory_val_gt[..., [2]], mask=input_trajectory_val_mask[..., [2]], lengths=input_trajectory_val_lengths, eot_startpos=input_trajectory_val_startpos[..., [3]], flag='Validation')
  val_depth_loss = DepthLoss(output=output_val_depth, depth_gt=output_trajectory_val[..., [0]], lengths=input_trajectory_val_lengths, mask=input_trajectory_val_mask)
  val_below_ground_loss = BelowGroundPenalize(output=output_val_xyz, trajectory_gt=output_trajectory_val_xyz[..., [0, 1, 2]], mask=output_trajectory_val_mask[..., [0, 1, 2]], lengths=output_trajectory_val_lengths)
  ...
  for epoch in range(start_epoch, n_epochs+1):
    ...
    for batch_idx, batch_train in enumerate(trajectory_train_dataloader):
      ...
      train_loss, val_loss, model_flag, model_depth = train(output_trajectory_train=output_trajectory_train, output_trajectory_train_mask=output_trajectory_train_mask,
                                                                 ...
                                                                 optimizer=optimizer, epoch=epoch, n_epochs=n_epochs, vis_signal=vis_signal, cam_params_dict=cam_params_dict)
