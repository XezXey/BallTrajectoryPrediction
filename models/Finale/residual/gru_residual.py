...
class GRUResidual(pt.nn.Module):
  def __init__(self, input_size, output_size, batch_size, model):
    super(GRUResidual, self).__init__()
  ...
  def forward(self, x, hidden, cell_state, lengths):
    ...
    for idx, recurrent_block in enumerate(self.recurrent_blocks):
      ...
      if idx == 0:
        ...
        out_packed, hidden = recurrent_block(out_packed, self.initHidden(self.batch_size))
        residual = self.get_residual(out_packed=out_packed, lengths=lengths, residual=residual, apply_skip=False)
      else:
        out_packed, hidden = recurrent_block(residual, self.initHidden(self.batch_size))
  ...
  def create_recurrent_block(self, in_f, hidden_f, num_layers, is_first_layer=False):
    if is_first_layer:
      return pt.nn.GRU(input_size=in_f, hidden_size=hidden_f, num_layers=num_layers, batch_first=True, bidirectional=False, dropout=0.)
    else :
