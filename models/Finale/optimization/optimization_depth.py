...
class TrajectoryOptimizationDepth(pt.nn.Module):
  def __init__(self, uv, cam_params_dict, input_dict, latent_size, model_dict, pred_dict, latent_code):
  ...
  def forward(self):
    ...
    in_f = pt.cat((self.uv, latent), dim=2)
    pred_depth, (_, _) = self.model_dict['model_depth'](in_f=in_f, lengths=self.input_dict['lengths'])
    return pred_depth

  def construct_latent(self):
    # This use EOT to split the latent. Stack and repeat to have the same shape with the "uv" 
    flag = self.pred_dict['model_flag']
    lengths = self.input_dict['lengths']
    ...
    for i in range(flag.shape[0]):
      ...
      if len(where) == 0:
        ...
        # latent = pt.nn.Parameter(pt.rand(1, self.latent_size, dtype=pt.float32).cuda(), requires_grad=True).cuda()
        self.latent.append(pt.nn.Parameter(pt.rand(1, self.latent_size, dtype=pt.float32).cuda() * 100., requires_grad=True).cuda())
      else:
        # Flag prediction work properly
        n_latent = len(where)
        # latent = pt.nn.Parameter(pt.rand(n_latent+1, self.latent_size, dtype=pt.float32).cuda(), requires_grad=True).cuda()
        self.latent.append(pt.nn.Parameter(pt.rand(n_latent+1, self.latent_size, dtype=pt.float32).cuda() * 100., requires_grad=True).cuda())
  ...
  def update_latent(self):
    ...
    for i in range(flag.shape[0]):
      ...
      # plt.plot(all_latent[i].cpu().detach().numpy(), 'r-o')
