...
class LSTM(pt.nn.Module):
  def __init__(self, input_size, output_size, batch_size, model):
    ...
    # Define the layers
    # LSTM layer
    self.recurrent_blocks = create_recurrent_block(in_f = self.input_size, hidden_f=self.hidden_dim, num_layers=self.n_layers)
    # FC
  ...
  def forward(self, x, hidden, cell_state, lengths):
    ...
    out_packed, (hidden, cell_state) = self.recurrent_blocks(out_packed, (self.initHidden(batch_size=self.batch_size), self.initCellState(batch_size=self.batch_size)))
