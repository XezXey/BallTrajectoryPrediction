...
class LSTM(pt.nn.Module):
  def __init__(self, input_size, output_size):
    ...
    self.fc_size = [self.hidden_dim, 64, 32, 16, 8, self.output_size]
    # Define the layers
    # RNN layer
    self.lstm = pt.nn.LSTM(input_size=self.input_size, hidden_size=self.hidden_dim, num_layers=self.n_layers, batch_first=True)
    # FC
  ...
  def forward(self, x, hidden, cell_state, lengths):
    ...
    out_packed, (hidden, cell_state) = self.lstm(x_packed)
